...
<body style="background-color: #F9FAFB; color: #2C3E50; font-family: 'Poppins', sans-serif;">
    ...
    <div class="p-4 sm:ml-64">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <button type="button" class="focus:outline-none text-white bg-blue-600 hover:bg-blue-700 font-medium rounded-full text-sm px-6 py-3 transition duration-300 shadow-md" onclick="openModal('add-pump')">Add Data</button>
        </div>
                ...
                <tbody>
                    ...
                    <tr class="bg-white hover:bg-[#FFF4E0] transition duration-300 border-b border-gray-300">
                        ...
                        <td class="px-6 py-4 font-medium text-black border-gray-300">
                            @if($item->img)
                                <img class="w-10 h-10 rounded-sm cursor-pointer shadow-md" src="{{ asset('assets/img/' . $item->img) }}" alt="Pump Image" onclick="openImageModal('{{ asset('assets/img/' . $item->img) }}')">
                            @else
            ...
            <div class="bg-white p-6 rounded-lg w-full sm:w-full md:w-1/2 lg:w-1/2 max-h-full overflow-y-auto shadow-lg">
                ...
                <form action="{{route('pump.store')}}" method="POST" enctype="multipart/form-data" id="add-form">
                    ...
                    <div class="flex justify-end mt-4">
                        <button type="button" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-full mr-2 transition duration-300" onclick="closeModal('add-pump')">Cancel</button>
        ...
        <div id="edit-pump-{{$item->id}}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto">
            <div class="bg-white p-6 rounded-lg w-full sm:w-full md:w-1/2 lg:w-1/2 max-h-full overflow-y-auto shadow-lg">
                ...
                <form action="{{ route('pump.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex justify-end mt-4">
                        <button type="button" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-full mr-2 transition duration-300" onclick="closeModal('edit-pump-{{$item->id}}')">Cancel</button>
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-full transition duration-300">Save</button>
                    </div>
                </form>
            </div>
        ...
        <div id="delete-pump-{{$item->id}}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white p-6 rounded-lg w-1/3 shadow-lg">
                <h2 class="text-xl font-bold mb-4">Delete Pump Data</h2>
                <p class="mb-4">Are you sure you want to delete this data?</p>
                <div class="flex justify-end">
                    <button type="button" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-full mr-2 transition duration-300" onclick="closeModal('delete-pump-{{$item->id}}')">Cancel</button>
