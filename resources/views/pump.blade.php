    ...
    @endif
        ...
        <div class="flex justify-center md:-mb-3">
            <div class="flex items-center space-x-3 md:justify-between md:space-x-40 lg:space-x-96">
                ...
                <button type="button" class="focus:outline-none text-white bg-blue-600 font-medium rounded-lg text-sm px-4 py-2 mb-4" onclick="openModal('add-pump')">Add Data</button>
            </div>
        </div>
        ...
        <table class="text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                ...
                <tr class="bg-white border-1 border-black">
                    ...
                    <td class="px-6 py-4 font-medium whitespace-nowrap border-1 border-black text-black">
                        @if($item->img)
                            <img class="w-10 h-10 rounded-sm cursor-pointer" src="{{ asset('assets/img/' . $item->img) }}" alt="Pump Image" onclick="openImageModal('{{ asset('assets/img/' . $item->img) }}')">
                        @else
        ...
        <div id="add-pump" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto">
            <div class="bg-white p-4 rounded-lg w-full h-1/2 sm:w-full md:w-1/2 lg:w-1/2 max-h-full overflow-y-auto">
                <h2 class="text-xl font-bold mb-4">Add Pump Data</h2>
                <form action="{{route('pump.store')}}" method="POST" enctype="multipart/form-data" id="add-form">
                    ...
                    <div class="flex justify-end">
                        <button type="button" class="bg-red-500 text-white px-4 py-2 rounded-md mr-2" onclick="closeModal('add-pump')">Cancel</button>
    ...
    <div id="edit-pump-{{$item->id}}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto">
        ...
        <div id="delete-pump-{{$item->id}}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white p-4 rounded-lg w-1/3">
                <h2 class="text-xl font-bold mb-4">Delet Pump Data</h2>
                <p class="mb-4">Are you sure you want to delete this data?</p>
                <div class="flex justify-end">
                    <button type="button" class="bg-red-500 text-white px-4 py-2 rounded-md mr-2" onclick="closeModal('delete-pump-{{$item->id}}')">Cancel</button>
